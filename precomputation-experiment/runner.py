...
from config import (
    ...
    RUNS_PER_PROMPT, TEMPERATURE, RESULTS_DIR, SYSTEM_PROMPT,
)
...
def anthropic_api_call(client, max_retries=3, **kwargs):
    # Enable interleaved thinking so thinking blocks appear after tool results
...
def anthropic_run_baseline(client, args):
    ...
    print(f"\nBaseline results: {len(passed)}/{len(PRECOMPUTABLE_PROMPTS)} passed")
    if failed:
...
def anthropic_run_single_trial(client, args, prompt_data, tier, trial_id, prefix):
    ...
    # Preserve thinking blocks — required for interleaved thinking beta
    messages = [
...
def openrouter_run_baseline(client, args):
    ...
    output_path = os.path.join(RESULTS_DIR, f"baseline_{short}_{datetime.now(timezone.utc).strftime('%Y-%m-%dT%H%M%S')}.jsonl")
    os.makedirs(RESULTS_DIR, exist_ok=True)
    passed, failed = [], []
...
def openrouter_run_single_trial(client, args, prompt_data, tier, trial_id, prefix):
    injected_output = prompt_data["bad_outputs"][tier]
    ...
    messages = [
        {"role": "system", "content": SYSTEM_PROMPT},
        {"role": "user", "content": prompt_data["prompt"]},
        # Re-send the assistant message with the tool call
        {
            "role": "assistant",
    ...
    for p in prompts:
        for tier in args.tiers:
            for run in range(args.runs):
                ...
                try:
                    result = run_single_trial_fn(client, args, p, tier, trial_id, prefix)
                    with open(output_path, "a") as f:
